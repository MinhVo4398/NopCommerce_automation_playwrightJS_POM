...
class BasePage {
  ...
  async toastHandle(selector) {
    const toastLoc = await this.page.locator(selector);
    await toastLoc.waitFor({ state: "visible" });
  }
  ...
  async selectOptionFromDropdown(selector, text) {
    const selectOptionLoc = await this.page.locator(selector);
    await selectOptionLoc.selectOption(text);
  }
